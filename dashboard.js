...
async function verifyPayment(reference) {
  try {
    const result = await callFunction({ action: "verify", reference });
...
async function addMessageNotification(u) {
  ...
  const messagesLink = [...nav.querySelectorAll("a")]
    .find(a => a.getAttribute("href") === "messages.html");
          ...
          return `
            <div class="listrow">
              <div>
                ...
                <div class="muted">Status: ${esc(p.status || "active")}</div>
                <div>
                  ${
                    activePromotion
                      ? `
                        <strong>${promotion === "premium" ? "Premium" : "Boosted"}</strong>
                        <div class="muted">
                          Expires: ${new Date(p.promotion_expires_at).toLocaleDateString()}
                        </div>
                      `
              ...
              ${
                activePromotion
                  ? ""
                  : `
                    <button class="btn small" data-pay="boost" data-id="${p.id}">
                      Boost ₦${BOOST_PRICE.toLocaleString()}
                    </button>
                    <button class="btn small" data-pay="premium" data-id="${p.id}">
                      Premium ₦${PREMIUM_PRICE.toLocaleString()}
                    </button>
                  `
              }

              <button data-id="${p.id}" class="btn small danger del">Delete</button>
            </div>
          `;
