...
function notice(m){
  noticeBox.textContent=m; noticeBox.style.display="block";
...
function val(r,keys,fb=""){
  for(const k of keys)if(r?.[k]!==undefined&&r?.[k]!==null&&r?.[k]!=="")return r[k];
  return fb;
}
/* FIX: admin_reports_overview returns { total, pending, reviewing, resolved, dismissed, recent:[...] }. */
function reportsArray(d){
  if(Array.isArray(d))return d;
  if(Array.isArray(d?.recent))return d.recent;
...
function cls(s){
  ...
  return s==="resolved"?"success":(s==="dismissed"||s==="rejected")?"danger":(s==="reviewing"||s==="in_review")?"active":"pending";
}
...
function renderReports(all){
  ...
  reportsList.innerHTML=rs.map(r=>{
    ...
    const type=val(r,["report_type","type","target_type"],"Report");
...
refreshBtn.addEventListener("click",async()=>{refreshBtn.disabled=true;refreshBtn.textContent="Loading...";try{await loadAdmin();await loadReports();}finally{refreshBtn.disabled=false;refreshBtn.textContent="Refresh";}});
loadReportsBtn.addEventListener("click",async()=>{loadReportsBtn.disabled=true;loadReportsBtn.textContent="Loading...";try{await loadReports();}finally{loadReportsBtn.disabled=false;loadReportsBtn.textContent="Refresh reports";}});
reportStatusFilter.addEventListener("change",loadReports);
notificationBtn.addEventListener("click",()=>{document.querySelector(".section")?.scrollIntoView({behavior:"smooth"});notice("Reports & Moderation notifications are shown in the Reports section.");});
logoutBtn.addEventListener("click",async()=>{logoutBtn.disabled=true;logoutBtn.textContent="Logging out...";const {error}=await supabase.auth.signOut();if(error){alert(error.message||"Could not log out.");logoutBtn.disabled=false;logoutBtn.textContent="Logout";return;}location.href="account.html";});
(async()=>{try{await loadAdmin();await loadReports();}catch(e){err(e.message||"Unable to initialize the admin dashboard.");}})();
setInterval(()=>{loadAdmin();loadReports();},30000);
