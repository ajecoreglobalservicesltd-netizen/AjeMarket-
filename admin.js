...
async function viewVerificationDocument(id,btn){
  ...
  try{
    ...
    const {data: rpcData,error: rpcError} = await supabase.rpc(
      "admin_get_seller_verification_document",
      {p_verification_id:id}
    );

    if(!rpcError && rpcData?.url){
      documentUrl = rpcData.url;
    }else{
      if(rpcError) console.warn("Document RPC failed; trying Storage fallback:", rpcError);
      ...
      if(!verification?.document_path){
        throw new Error("No verification document is stored for this request.");
      }
      ...
      if(!signedData?.signedUrl){
        throw new Error("Could not create a secure document URL.");
      }
    ...
    console.error("Verification document error:",e);
    err(e.message||"Could not open verification document.");
  }finally{
    btn.disabled=false;
    btn.textContent="View verification document";
  }
}
...
async function updateSellerVerification(id,status,btn){
  ...
  try{
    ...
    if(!data?.success){
      throw new Error(
        data?.message ||
        "Seller verification update was not confirmed by the database."
      );
    }
