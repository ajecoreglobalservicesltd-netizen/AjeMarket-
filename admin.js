...
async function loadNotifications(){
  try{
    const s=await session(); if(!s)return;
    const {data,error}=await supabase.rpc("admin_notifications_owner");
    ...
    notificationList.querySelectorAll(".notification-item").forEach(item=>item.addEventListener("click",async()=>{
      const {error}=await supabase.rpc("admin_mark_notification_read_owner",{p_notification_id:item.dataset.id});
...
async function markAllNotificationsRead(){
  ...
  try{
    const {error}=await supabase.rpc("admin_mark_all_notifications_owner");
    if(error)throw error;
