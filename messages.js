...
function statusMarkup(message) {
  if (!currentUser || String(message.sender_id) !== String(currentUser.id)) return "";
...
async function markMessagesDelivered(conversationId) {
  ...
  if (error) {
    console.warn("Delivery update:", error.message);
  }
}
...
async function markMessagesRead(conversationId) {
  ...
  if (error) {
    console.warn("Read update:", error.message);
  }
}

async function markConversationAsSeen(conversationId) {
...
async function showConversations() {
  ...
  const sorted = [...data].sort((a, b) => {
    const at = new Date(
      latestTimes.get(a.id) || a.updated_at || a.created_at
    ).getTime();

    const bt = new Date(
      latestTimes.get(b.id) || b.updated_at || b.created_at
    ).getTime();
    ...
    <div class="messages-box">
      ...
      <div class="conversation-list">
        ${sorted.map(c => {
          ...
          const personName =
            profile?.full_name || "AjeMarket user";
          ...
          return `
            <a class="conversation-item ${unread ? "has-unread" : ""}"
              ...
              <div class="conversation-main">
                ...
                ${
                  product?.price != null
                    ? `<small class="muted">${money(product.price)}</small>`
                    : ""
                }
  ...
  const totalUnread = [...unreadCounts.values()].reduce(
    (sum, n) => sum + n,
    0
  );

  document.title = totalUnread
    ? `(${totalUnread}) Messages — AjeMarket`
    : "Messages — AjeMarket";
}
...
async function openConversation(conversationId) {
  ...
  const isBuyer =
    String(conversation.buyer_id) === String(currentUser.id);

  const isSeller =
    String(conversation.seller_id) === String(currentUser.id);
  ...
  /*
    ...
    1. Messages from the other person are delivered.
    2. Messages from the other person are seen/read.
  */
  ...
  /*
    Keep the conversation marked as seen while the page is open.
    This matters if the other person sends another message while
    the recipient is already inside this chat.
  */
...
async function handleVisibilityChange() {
  ...
  if (document.visibilityState === "visible") {
    await markConversationAsSeen(currentConversation.id);
    await loadMessages();
  }
}
...
async function loadMessages() {
  ...
  let result = await supabase
    .from("messages")
    .select(
      "id,conversation_id,sender_id,message,is_read,delivered_at,read_at,created_at"
    )
    .eq("conversation_id", currentConversation.id)
    .order("created_at", { ascending: true });

  /*
    Compatibility fallback for databases where the status columns
    have not been created yet.
  */
  if (result.error) {
    console.warn(
      "Full message status query failed:",
      ...
      return `
        <div class="chat-message ${mine ? "mine" : "theirs"}">
          ...
          <div class="message-meta">
            <small class="muted">
              ${esc(formatTime(message.created_at))}
            </small>
...
function subscribeToMessages() {
  ...
  messageChannel = supabase
    ...
    /*
      New message:
      - Recipient gets it.
      - Recipient immediately marks it delivered/read if
        the chat is currently visible.
      - Both sides reload so the sender sees ✓✓.
    */
    .on(
      "postgres_changes",
      {
    ...
    /*
      Status changes:
      - delivered_at changes => sender sees ✓✓.
      - is_read/read_at changes => sender sees green ✓✓.
    */
    .on(
      "postgres_changes",
      {
...
async function sendMessage(event) {
  ...
  /*
    ...
    The recipient's device will set delivered_at and
    read_at/is_read when it receives/opens the chat.
  */
  const { error } = await supabase
    .from("messages")
    .insert({
  ...
  if (error) {
    /*
      Compatibility fallback in case the existing database
      does not yet allow the status columns on INSERT.
    */
    ...
    if (fallback.error) {
      alert("Message failed: " + fallback.error.message);
