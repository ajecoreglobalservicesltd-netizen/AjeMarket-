...
async function markMessagesRead(conversationId) {
  ...
  // A message is read only when the conversation is actually open.
  // read_at IS NULL also handles rows where is_read is NULL.
  const { error } = await supabase
    .from("messages")
    .update({
      is_read: true,
...
async function markAllVisibleConversationsDelivered(conversations) {
  if (!currentUser || !conversations?.length) return;

  // Being on the AjeMarket messages page means the account is online/available.
  // Mark received messages as delivered, but NOT read.
  await Promise.all(
    conversations.map(c => markMessagesDelivered(c.id))
  );
}
...
async function showConversations() {
  ...
  // Online on the messages page = delivered, but not read.
  await markAllVisibleConversationsDelivered(data);
...
async function openConversation(conversationId) {
  ...
  // Entering the actual conversation means the received messages are read.
    ...
    .on(
      ...
      async payload => {
        if (payload?.new &&
          ...
          // Recipient is already inside this conversation, so the new message
          // is delivered and read immediately.
