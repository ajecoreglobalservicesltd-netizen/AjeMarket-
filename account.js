...
function loginUI() {
  root.innerHTML = `
    ...
    <form id="auth">
      ...
      <button type="button" id="forgotPassword" class="link-button">Forgot password?</button>
  ...
  forgotButton.onclick = async () => {
    ...
    if (!email) {
      msg.textContent = "Enter your email address first, then tap Forgot password.";
      emailInput.focus();
      return;
    }
    ...
    try {
      ...
      msg.textContent = "Password reset email sent. Check your email and follow the link to create a new password.";
    } catch (error) {
      console.error(error);
  ...
  form.onsubmit = async event => {
    ...
    if (loginError) {
      msg.textContent = "Account created. Please check your email if confirmation is required.";
      return;
    }

    if (loginData?.session) location.href = "dashboard.html";
  };
}
...
function passwordResetUI() {
  ...
  root.innerHTML = `
    ...
    <form id="resetForm">
      ...
      <button class="btn full" type="submit" id="resetButton">Update password</button>
  ...
  form.onsubmit = async event => {
    ...
    msg.textContent = "Password updated successfully. You can now sign in with your new password.";
...
async function loadFavorites(u) {
  ...
  section.innerHTML = `
    <div class="section-title">
      <div><small>SAVED PRODUCTS</small><h2>My Favourites</h2></div>
    </div>
    <div id="favoritesList" class="grid"><div class="empty">Loading your favourites…</div></div>
  `;
  ...
  if (favoriteError) {
    list.innerHTML = `<div class="empty">Could not load your favourites.<br><br>${esc(favoriteError.message)}</div>`;
    return;
  }
  ...
  if (!favorites || favorites.length === 0) {
    list.innerHTML = `
      <div class="empty">
        ...
        <p class="muted">Tap <b>Save</b> on a product you like and it will appear here.</p>
  ...
  if (productError) {
    list.innerHTML = `<div class="empty">Could not load your saved products.<br><br>${esc(productError.message)}</div>`;
    return;
  }
      ...
      return `
        <div class="card">
          <div class="cardbody">
            ...
            <p class="muted">This saved product is no longer available.</p>
            <button class="btn danger small remove-favorite" data-id="${esc(favorite.product_id)}">Remove</button>
          </div>
        </div>
      `;
    }

    const title = product.title || product.name || "AjeMarket product";
    const price = product.price !== undefined && product.price !== null ? money(product.price) : "Price unavailable";
    const location = product.location || product.city || "";
      ...
      <article class="card">
        ...
        <div class="cardbody">
          ...
          <div class="actions" style="margin-top:14px">
            <a class="btn small" href="product.html?id=${encodeURIComponent(product.id)}">View product</a>
            <button class="btn danger small remove-favorite" data-id="${esc(product.id)}">Remove</button>
          </div>
        </div>
      </article>
    `;
...
async function accountUI(u) {
  ...
  root.innerHTML = `
    ...
    <div class="actions">
      ...
      <a class="btn" href="seller-verification.html">🛡️ Seller Verification</a>
      <button class="btn danger" id="out">Sign out</button>
    </div>

    <div class="notice" style="margin-top:18px;">
      <b>Seller trust</b><br>
      ${p?.is_verified_seller ? '<span style="color:#00c98b;font-weight:900;">✓ Trusted Seller</span><br><span class="muted">Your seller account has been verified by AjeMarket.</span>' : `<span class="muted">Status: ${esc(p?.verification_status || "not_verified")}</span><br><a class="btn small" href="seller-verification.html" style="margin-top:10px;display:inline-block;">Get verified</a>`}
    </div>
  ...
  if (recoveryDetected) {
    ...
    root.innerHTML = `
      <div class="empty">
        ...
        <p class="muted">Please request another password reset email.</p>
        <button class="btn" id="backToLogin">Back to sign in</button>
      </div>
    `;
