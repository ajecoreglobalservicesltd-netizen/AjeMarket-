...
async function load() {
    ...
    ? `
      <div class="aje-gallery">
        <div class="aje-main-image">
          <img id="mainProductImage" src="${esc(firstImage)}"
  ...
  root.innerHTML = `
    <div class="product">
      ...
      <div class="panel">
        ...
        <hr>
        <h3>Seller</h3>
        <p><b>${esc(sellerName)}</b>${profile?.verified ? " ✓ Verified" : ""}</p>
  ...
  document.querySelectorAll(".aje-thumbnail").forEach(button => {
    button.onclick = () => {
      ...
      document.querySelectorAll(".aje-thumbnail").forEach(item => {
  ...
  if (report) {
    report.onclick = async () => {
      ...
      try {
        ...
        alert(
          "Report submitted successfully. AjeMarket will review this listing."
        );
